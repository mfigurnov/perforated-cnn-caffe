...
template <typename Dtype>
class ConvolutionPerforatedLayer : public Layer<Dtype> {
 public:
  /**
   * @param param provides ConvolutionParameter convolution_param,
   *    with ConvolutionLayer options:
   ...
   *    kernels + stream parallelism) engines.
   */
  explicit ConvolutionPerforatedLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "ConvolutionPerforated"; }
 ...
 protected:
  ...
  int non_perforated_out_;  // number of non-perforated spatial positions
  int micro_batch_size_;  // number of images to stack in im2col
  bool bias_term_;
  ...
  Blob<Dtype> col_buffer_;
  Blob<Dtype> bias_multiplier_;
  Blob<Dtype> top_buffer_, top_buffer_2_;
  Blob<unsigned int> non_perforated_indices_;
  Blob<unsigned int> interpolation_indices_;
};
